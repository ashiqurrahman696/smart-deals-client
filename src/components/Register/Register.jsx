...
const Register = () => {
    ...
    const handleRegister = e => {
        ...
        else if(!imageUrlPattern.test(image)){
            toast.error("Invalid image URL");
            return;
        }

        else if(!min8Pattern.test(password)){
            toast.error("Password must be at least 8 characters long");
            return;
        }

        else if(!noSpacePattern.test(password)){
        ...
        <div className="min-h-screen flex justify-center items-center bg-neutral-100">
            <div className="card bg-base-100 w-full max-w-sm shrink-0 shadow-2xl">
                <div className="card-body">
                    <h2 className="text-center font-semibold text-4xl">Register Now!</h2>
                    <p className="text-center">Already have an account? <Link to="/login" className="bg-linear-to-br from-[#632EE3] to-[#9F62F2] text-transparent bg-clip-text">Login Now</Link></p>
                    <form onSubmit={handleRegister} className="fieldset">
                        <div>
                            <label className="label">Name</label>
                            <input type="text" name="name" className="input w-full mt-1" placeholder="Name" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="image" className="input w-full mt-1" placeholder="Image URL" />
                        </div>
                        ...
                        <button className="btn text-base-100 mt-4 bg-linear-to-br from-[#632EE3] to-[#9F62F2]">Register</button>
